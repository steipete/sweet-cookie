...
export async function getCookies(options: GetCookiesOptions): Promise<GetCookiesResult> {
	...
	for (const browser of browsers) {
		...
		if (browser === 'chrome') {
			...
			const arcOptions: Parameters<typeof getCookiesFromArc>[0] = {};
			const arcProfile =
				options.arcProfile ?? options.profile ?? readEnv('SWEET_COOKIE_ARC_PROFILE');
			if (arcProfile) arcOptions.profile = arcProfile;
			if (options.timeoutMs !== undefined) arcOptions.timeoutMs = options.timeoutMs;
			if (options.includeExpired !== undefined) arcOptions.includeExpired = options.includeExpired;
			if (options.debug !== undefined) arcOptions.debug = options.debug;
...
function parseBrowsersEnv(): GetCookiesOptions['browsers'] | undefined {
	...
	const tokens = raw
		...
		.filter(Boolean);
	const out: Array<'arc' | 'chrome' | 'edge' | 'firefox' | 'safari'> = [];
